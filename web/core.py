...
SECTION_SCHEMA: dict[str, dict[str, tuple]] = {
    ...
    'Testing': {
        'timeout': (
            ...
            '测试超时(秒)',
            '单个源探测的最长等待时间，超时即判失败',
            [('5', '5秒'), ('10', '10秒(推荐)'), ('20', '20秒'), ('30', '30秒'), ('60', '60秒')],
        ),
        'concurrent_threads': (
            'int',
            '50',
            '并发线程数',
            '同时探测的源数量，越大越快但更占资源',
            [('10', '10'), ('20', '20'), ('50', '50(默认)'), ('100', '100'), ('200', '200'), ('300', '300')],
        ),
...
async def lifespan(app_instance: FastAPI):
    ...
    if admin_pw is not None and not models.password_complexity_ok(admin_pw):
        # 仅当用户显式设置了密码且不满足复杂度时才拒绝启动；未设置则由 init_db 使用默认初始密码 Admin@123
        # （首次部署零配置即可用，默认密码满足合规复杂度，首次登录强制修改）
        # 复杂度规则统一由 web.models.password_complexity_ok 判定（单一来源）
        _cats = sum(
            [
                bool(re.search(r'[A-Z]', admin_pw)),
                bool(re.search(r'[a-z]', admin_pw)),
                bool(re.search(r'[0-9]', admin_pw)),
                bool(re.search(r'[^A-Za-z0-9]', admin_pw)),
            ]
        )
        raise RuntimeError(
            f'【合规拒绝启动】密码不满足GB/T 39786-2021复杂度要求。\n'
            f'  当前长度: {len(admin_pw)}, 包含字符类别数: {_cats}（至少需要3类，长度≥8）\n'

...
WEB_PORT="${WEB_PORT:-23455}"                       # Web管理界面端口
...
setup_nginx() {
    ...
    # 注入NGINX_PORT环境变量到nginx配置（纪枢 C-4）
    if [ "$NGINX_PORT" != "12345" ]; then
        log_info "替换Nginx监听端口为: ${NGINX_PORT}"
        sed -i "s/listen 12345/listen ${NGINX_PORT}/g" /etc/nginx/nginx.conf
        sed -i "s/listen \[::\]:12345/listen \[::\]:${NGINX_PORT}/g" /etc/nginx/nginx.conf
    fi
...
setup_cron_jobs() {
    ...
    echo "$UPDATE_CRON cd /app && $python_path main.py >> /log/cron.log 2>&1" >> "$cron_file"
    echo "# 结束" >> "$cron_file"
...
health_check() {
    ...
    # 检查FFmpeg/FFprobe是否可用，不可用则尝试自动安装（远山总要求）
    if ! command -v ffprobe >/dev/null 2>&1; then
        log_warn "FFprobe不可用，尝试自动安装FFmpeg..."
        if apt-get install -y --no-install-recommends ffmpeg >/dev/null 2>&1; then
            log_info "✓ FFmpeg安装成功"
            apt-get clean && rm -rf /var/lib/apt/lists/*
        else
            log_warn "FFmpeg自动安装失败，流媒体测试功能将受限"
        fi
    else
...
main() {
    ...
    if start_main_program; then
        ...
        # C3: 不能 exit 1 导致容器退出，继续保持运行等待定时任务重试

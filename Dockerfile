...
# Debian bookworm (12+) 使用 .sources DEB822 格式，需替换 URIs 字段而非简单的域名替换
RUN ( \
    if [ -f /etc/apt/sources.list.d/debian.sources ]; then \
        sed -i 's|URIs: http://deb.debian.org|URIs: http://mirrors.tuna.tsinghua.edu.cn|g' \
...
# 增加 extra-index-url 官方源作为备用，避免清华源同步延迟导致个别依赖安装失败
...
# 多源容错：清华镜像（国内优先）→ 中科大镜像（国内备用）→ johnvansickle（海外主源）→ GitHub（最后备用）

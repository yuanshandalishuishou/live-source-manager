...
class StreamTester:
    ...
    def _verify_ffprobe(self):
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                self.logger.warning("⚠ FFprobe执行返回非零，流质量检测将受限")
                self.ffprobe_available = False
        except (subprocess.TimeoutExpired, FileNotFoundError, Exception) as e:
            self.logger.warning(f"⚠ FFprobe工具不可用: {e}，流质量检测将受限")
            self.ffprobe_available = False
        ...
        try:
            ...
            for attempt in range(self.max_retries + 1):
                if attempt > 0:
                    ...
                    self.logger.info(
                        f"重试 #{attempt}/{self.max_retries} [{source.get('name', '')}] "
    ...
    def _watchdog_timeout_handler(self):
        ...
        self.logger.error(
            f"⚠ 看门狗触发！批量测试已执行超过 {self._watchdog_timeout}s，"
            "正在运行中的测试将被标记为超时"
        )
